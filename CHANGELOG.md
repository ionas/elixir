...
The `if` block above is implicitly rebinding `message` to a new value. Now imagine we want to move the `if` block to its own function to clean up the implementation:
